class ApplicationController < ActionController::Base
  ...
  def render_404
    if @cms_config['site_settings']['old_domain'] && Net::HTTP.get_response(URI.parse("#{@cms_config['site_settings']['old_domain']}"+"#{request.request_uri}")).code == '200'
      Redirect.create(:old_path => request.url, :new_path => "#{@cms_config['site_settings']['old_domain']}"+"#{request.request_uri}")
      redirect_to Redirect.find(:first, :conditions => "old_path = '#{request.url}'").new_path, :status => 301 and return
    else
  ...
  def get_siteninja_config
    ...
    unless @cms_config["site_settings"]["private"] or RAILS_ENV == "development" or RAILS_ENV == "test"
